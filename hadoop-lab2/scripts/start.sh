...
for i in {1..200}
	do
		...
		sudo -u postgres -H -- psql -d $1 -c 'INSERT INTO input1 (senderUser, receiverUser, datetime, msg) values ('"${USERPREFIX}${USER}"','"${USERPREFIX}${USER}"','\''May 03 '"$TWO_DIGIT_HOUR"':13:56'\'','\'"${POSTFIX[$((RANDOM % ${#POSTFIX[*]}))]}"''\'');'
	done
